...
namespace RenderInfoRepoGitHubToListView.Services
{
    public class GitHubRepoCrawlerService
    {
        public async Task<List<Repo>> SearchRepoGitHub(string search, int take)
        {
            ...
            {
                if (GetStringValueFor("name", line, out var name))
                {
                ...
                else if (GetStringValueFor("forks_count", line, out var fork))
                {
